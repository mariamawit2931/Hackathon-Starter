...
import { ArrowRight, ArrowLeft, Zap, TrendingUp, CheckCircle, AlertCircle } from "lucide-react";
...
export default function Assessment() {
  ...
  const startProcessing = async () => {
    // 1. SAVE DATA FOR THE RESULTS PAGE
    const profileData = {
    ...
    try {
      ...
      setError("Server Connection Failed. Is the Backend running on Port 3000?");
      ...
      <div className="min-h-[80vh] flex items-center justify-center px-4 bg-black text-white">
        <div className="text-center max-w-md w-full">
          <div className="relative w-24 h-24 mx-auto mb-8">
            <TrendingUp className="w-12 h-12 text-primary animate-pulse absolute inset-0 m-auto" />
          ...
          <h2 className="text-2xl font-bold mb-6 font-['Space_Grotesk']">BizPath AI Analyzing...</h2>
    ...
    <div className="min-h-[80vh] py-12 px-4 bg-black">
      <div className="max-w-2xl mx-auto">
            ...
            <AnimatePresence mode="wait">
              {currentStep === 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                  <h2 className="text-2xl font-bold text-white">How much capital do you have?</h2>
                  <div className="text-center py-6 bg-white/5 rounded-2xl border border-white/5">
                    ...
                    <Slider value={[capital]} onValueChange={v => setCapital(v[0])} min={5000} max={500000} step={5000} className="my-10" />
              ...
              {currentStep === 1 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {COUNTRIES.map(c => (
                      <Button key={c} variant={selectedCountry === c ? "default" : "outline"} onClick={() => setSelectedCountry(c)}>{c === "Ethiopia" ? "🇪🇹 " : ""}{c}</Button>
                    ))}
                  </div>
              ...
              {currentStep === 2 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                  ...
                  <div className="space-y-4">
                    <p className="text-xs text-white/40 uppercase tracking-widest font-bold">Your Professional Skills</p>
                    <div className="flex flex-wrap gap-2">
                      {SKILLS.map(s => (
                        <Badge key={s} variant={selectedSkills.includes(s) ? "default" : "outline"} className="cursor-pointer py-2 px-4 rounded-full" onClick={() => toggleSkill(s)}>{s}</Badge>
                      ))}
                    </div>
                  </div>
                  <div className="space-y-4 pt-4 border-t border-white/5">
                    <p className="text-xs text-white/40 uppercase tracking-widest font-bold">What you love</p>
                    <div className="flex flex-wrap gap-2">
                      {PASSIONS.map(p => (
                        <Badge key={p} variant={selectedPassions.includes(p) ? "secondary" : "outline"} className="cursor-pointer py-2 px-4 rounded-full" onClick={() => togglePassion(p)}>{p}</Badge>
                      ))}
                    </div>
                  </div>
                </motion.div>
        ...
        <div className="flex justify-between items-center">
          ...
          <Button size="lg" className="bg-emerald-500 hover:bg-emerald-600 text-black font-bold px-8" onClick={handleNext} disabled={currentStep === 2 && (selectedSkills.length < 1 || selectedPassions.length < 1)}>
